...
model = TransferModel((320, 160, 3))
model.build(input_shape=(None, 320, 160, 3))
...
van_gogh = tf.image.resize(van_gogh, size=(320, 160))
...
plt.axis('off')
plt.savefig('data/styled_van_gogh.png', dpi=80 ,bbox_inches='tight', pad_inches = 0)
plt.close()

# To make movie
import glob

img_paths = glob.glob('/home/qbeer/pizza_images/*.png')
...
for img_path in img_paths:
    ...
    img = tf.image.resize(img, size=(320, 160))
    img = tf.expand_dims(img, axis=0)
    styled_img, _ = model(img)
    plt.imshow(deprocess_input(styled_img[0]))
    plt.axis('off')
    plt.savefig(img_path.replace('.png', '_styled.png'), dpi=80, bbox_inches='tight', pad_inches=0)
    plt.close()